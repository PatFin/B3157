...
static facteur * get_prime_factors ( uint64_t nombre, uint64_t * i, uint64_t * inc )
// Mode d'emploi :
...
//	l'arbre.
{
	...
	else
	{
		...
		if ( nombre % 2 == 0 )
		{
			...
			nouvelleFeuille->facteur = get_prime_factors ( nombre / 2, i, inc );
		...
		if ( nombre % 3 == 0 )
		{
			...
			nouvelleFeuille->facteur = get_prime_factors ( nombre / 3, i, inc );
		...
		while ( (*i) * (*i) < nombre )
		{
			if ( nombre % (*i) == 0 )
			{
				...
				nouvelleFeuille->facteur = get_prime_factors ( nombre / (*i), i, inc );	//Appel récursif;
				Insert ( nouvelleFeuille, retour );
...
void threadMain ( void * thrData )
{
	...
	while ( EOF != (fscanf ( lecture, "%" PRIu64 "", &nombre ) ) )
	{
		...
		uint64_t i = 5;
		uint64_t inc = 4;
		nombreCalcule = get_prime_factors ( nombre, &i, &inc );
...
int main ( void )
{
	...
	pthread_create( &thread3, NULL, (void *) &threadMain, NULL );
	pthread_create( &thread4, NULL, (void *) &threadMain, NULL );
//------------------------------PHASE MOTEUR -----------------------------------
